...
async function generateProPDF(deckName) {
  ...
  await page.evaluate(() => {
    ...
    cards.forEach((card, index) => {
        ...
        // Apply page break to all except the very last one
  ...
  const outputPath = path.join(__dirname, 'print_exports', `${deckName.replace(/ /g, '_')}_Press_Sheet.pdf`);
...
async function run() {
  ...
  await generateProPDF('Full Set (1x each)');
}
